...
gameEngine::gameEngine()
{
    ...
    //初始化文本
    main_interface1=new textShower("*主界面的测试文本",background);
...
void gameEngine::keyPressEvent(QKeyEvent *k)
{
    ...
    if(!failed)
    {
        ...
        if(is_players_turn)
        {
            if(k->key()==Qt::Key_Right  &&!is_showing_target&&!is_fighting&&!bar_moving&&!is_selecting&&!acted&&!is_showing_item&&!is_using_item&&!used_item)
            {
                select->play();
                button_index++;
            ...
            if(k->key()==Qt::Key_Left  &&!is_showing_target&&!is_fighting&&!bar_moving&&!is_selecting&&!acted&&!is_showing_item&&!is_using_item&&!used_item)
            {
                select->play();
                button_index--;
...
void gameEngine::show_dialog(QString str)
{
    ...
    connect(dialog_timer,&QTimer::timeout,this,[=]()
    {
        ...
        //dia_sound->stop();
        //dia_sound->play();
        dia[i]->setPos(dialog_bubble->x()+26+17*i,dialog_bubble->y()+11);
...
void gameEngine::battle()
{
    ...
    if(turn==1)
    {
        ...
        for(int i=0;i<=9;i++)
        {
            dbs[i]=new deerBlaster(battle_frame->soul);
        }
    ...
    if(turn==6)
    {
        ...
        //for(int i=1;i<=25;i++)
            //dbs[i]->show_db(320-32+150*cos(15*(i-1)),240-45+150*sin(15*(i-1)),15*(i-1),background);
